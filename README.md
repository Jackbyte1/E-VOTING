...
Local development can simulate email delivery with `OTP_DELIVERY_MODE=console`. For Render or a real election, use `OTP_DELIVERY_MODE=smtp` and configure SMTP credentials. Keep `EXPOSE_DEV_OTP=false` on Render so OTP codes are never returned to the browser response.
...
SMTP_PASSWORD=<your email provider app password or SMTP password>
SMTP_USE_TLS=true
SMTP_USE_SSL=false
...
For Gmail, enable two-factor authentication on the sender account and create an App Password. Use that App Password as `SMTP_PASSWORD`; do not use your normal Gmail login password.
