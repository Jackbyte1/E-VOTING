...
def record_audit(action: str, user_id=None, merkle_root=None) -> str:
    ...
    with get_connection() as conn:
        ...
        current_hash = sha256_hash(f"{action}|{user_id}|{timestamp}|{previous_hash}|{merkle_root}")
        conn.execute(
            """
            INSERT INTO audit_logs (action, user_id, timestamp, previous_hash, current_hash, merkle_root)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
